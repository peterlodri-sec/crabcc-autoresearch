...
def test_run_end():
    ...
    resp = client.post(
        "/api/runs/end", json={"run_id": "t-end-1", "total_cost_usd": 4.23}
    )
    assert resp.status_code == 200
...
def test_run_end_unknown_run_returns_404():
    resp = client.post(
        "/api/runs/end", json={"run_id": "no-such-run", "total_cost_usd": 0.0}
    )
...
def test_list_sessions():
    client.post(
        "/api/runs/start",
        json={"run_id": "t-list-1", "gpu_type": "A100", "provider": "runpod"},
    )
...
def test_sessions_include_best_val_bpb():
    ...
    client.post(
        "/api/telemetry",
        json={"run_id": "t-best-1", "step": 1, "val_bpb": 2.5, "status": "SUCCESS"},
    )
    client.post(
        "/api/telemetry",
        json={"run_id": "t-best-1", "step": 2, "val_bpb": 2.1, "status": "SUCCESS"},
    )
    resp = client.get("/api/sessions")
